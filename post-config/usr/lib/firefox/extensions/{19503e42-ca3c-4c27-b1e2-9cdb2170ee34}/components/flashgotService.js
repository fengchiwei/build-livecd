...
FlashGotDMCust.prototype.makeArgs = function(URL, REFERER, COOKIE, FOLDER, POST,  CFILE, UFILE) {
  ...
  for(var m; 
    ...
    v = parms[m[4]];
    len = typeof(v) != "undefined" && v != null ? m.length : 2;
    /* // to be investigated deeper
    if(isWin && v) {
      v = this.sanitizeWinArg(v);
    }
    */
    for(j =1 ; j < len; j++) {
      if(j == 3) {
        args[args.length] = m[3] + v + m[5];
        j = 5;
      } else {
        s=m[j];
        if(s.length) {
          this._addParts(args, s);
        }
      }
    }
  }
...
FlashGotDM.initDMS=function(service) {
  ...
  dm.createJob = function(links, opType) {
    const service = this.service;
    const shellEsc = this.shellEsc;
    const referer = shellEsc(this.getReferrer(links));
...
function FlashGotService() {
  ...
  osvr.addObserver(this.interceptor, "http-on-modify-request", false);
